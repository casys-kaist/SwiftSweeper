...
if [ -z "$LIBCS" ]; then
    LIBCS="glibc SwiftSweeper ffmalloc markus"
fi
...
function generate_benches() {
    ...
    local benches="parsec.blackscholes parsec.bodytrack parsec.canneal parsec.dedup parsec.facesim parsec.ferret parsec.fluidanimate parsec.freqmine parsec.netdedup parsec.netferret parsec.netstreamcluster parsec.streamcluster parsec.swaptions parsec.vips parsec.x264"
    local libc="$1"
    local thread="$2"
...
do  
    ...
    do
        ...
        mkdir -p ../result/parsec/${LIBC}_${THREAD}t
        sudo mv ../result/parsec/*.result ../result/parsec/*.time.out* ../result/parsec/${LIBC}_${THREAD}t
    done
done