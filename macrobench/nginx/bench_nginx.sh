...
source ../server_conf
SERVER_COMMAND="cd SwiftSweeper-User/macrobench/common; ./run_apache_nginx_client.sh nginx \"${NUM_CONNECTIONS}\" ${NUM_THREADS} ${BENCH_SEC}"
...
do
    ...
    # Need some rest...
    echo "---------------------------------------------------------------"
    echo "Prepare next bench: sleep 10 sec.."
    echo "---------------------------------------------------------------"
    sleep 10
done
