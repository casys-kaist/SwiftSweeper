...
if __name__ == "__main__":
    ...
    # define bench target
    BENCH_TARGETS=["glibc", "SwiftSweeper", "hushvac", "markus", "minesweeper"]
    ...
    with open(os.path.join(output_dir, "results.csv"), "w") as results_csv:
        ...
        for cve_target_path, cve_target_name in zip(cve_targets_path, cve_targets_name):
            ...
            for BENCH_TARGET in BENCH_TARGETS:
                ...
                try :
                    # All process will be end within 30 seconds, except infinite loop
                    result = subprocess.check_output([f'{run_script} {BENCH_TARGET}'], shell=True, timeout=30)
